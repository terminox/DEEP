...
// Vendored from https://github.com/nikstar/VariableBlur, which itself credits
// https://github.com/jtrivedi/VariableBlurView.
//
// ⚠️ Uses the PRIVATE `CAFilter("variableBlur")`, reached via obfuscated runtime
// string lookup + KVC so the symbol is never linked directly. This can risk App
// Store review rejection — kept isolated to this one file so it can be swapped
// or removed wholesale if needed.
...
public struct VariableBlurView: UIViewRepresentable {
  ...
  /// to `maxBlurRadius`. Setting `startOffset` to a small negative coefficient
  /// (e.g. -0.1) will start blur from a larger radius value which might look
...
open class VariableBlurUIView: UIVisualEffectView {
  ...
  public init(maxBlurRadius: CGFloat = 20, direction: VariableBlurDirection = .blurredTopClearBottom, startOffset: CGFloat = 0) {
    ...
    // corresponding pixel in the gradient mask. An alpha of 1 results in the max
    // blur radius, while an alpha of 0 is completely unblurred.
    ...
    // `CABackdropLayer`, which is able to apply various, real-time CAFilters onto
    // the views underneath.
  ...
  open override func didMoveToWindow() {
    ...
    // (https://github.com/nikstar/VariableBlur/issues/1)
