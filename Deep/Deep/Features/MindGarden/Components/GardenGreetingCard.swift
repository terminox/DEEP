...
struct GardenGreetingCard: View {
  ...
  private var cherishButton: some View {
    Button {
      withAnimation(DeepMotion.settle) { isCherished.toggle() }
    } label: {
      Image(systemName: isCherished ? "heart.fill" : "heart")
        .font(.system(size: 16, weight: .semibold))
        .foregroundStyle(isCherished ? DeepColor.blushPowder : DeepColor.driftGrey)
    ...
    VStack(alignment: .leading, spacing: 8) {
      ...
      GeometryReader { geo in
        ZStack(alignment: .leading) {
          Capsule().fill(DeepColor.lavenderMist.opacity(0.18))
          Capsule()
            .fill(LinearGradient(
  ...
  private var streakStat: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      HStack(spacing: 6) {
        Image(systemName: "flame.fill")
          ...
          .foregroundStyle(LinearGradient(
            colors: [DeepColor.peachCloud, DeepColor.blushPowder],
      ...
      Text("Best yet — keep it alight")
        .font(DeepType.caption)
        .foregroundStyle(DeepColor.driftGrey.opacity(0.85))
    }
  }
}
