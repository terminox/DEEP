...
///
/// Three rows and no symbol column — every row here is the same kind of
...
struct LanguageView: View {
  ...
  private func select(_ language: AppLanguage) {
    guard languageStore.selection != language else { return }
...
#if DEBUG
#Preview("Language — following the device") {
  NavigationStack {
    LanguageView()
